...
import { claveDePalabra, registrarAciertoPalabra, registrarFalloPalabra, registrarPuntuacion, registrarResultadoPartida, obtenerPartidaGuardada, borrarPartidaGuardada } from './modulos/persistencia.js';
import { initInterfazMenu, refrescarProgresoVocabulario, abrirAjustesDesdePausa, prepararPartida } from './modulos/interfazMenu.js';
import { obtenerAjuste, fijarAjuste, alCambiarAjuste } from './modulos/ajustes.js';
import { guardarPartidaActual, restaurarSnapshotPartida } from './modulos/partida.js';
...
posicionarJugador(); 
  ...
  state.MAX_ENEMIES = obtenerAjuste('maxEnemigos') ?? 6;
  state.tiempoPartidaMs = 0;
  state.ultimoTiempoPartidaMs = null;
  state.ultimoGuardadoPartidaMs = 0;
  state.estadisticasPartida = { fases: [], hitos: [] };
...
function spawnEnemy() {
  ...
  const multiplicadoresVelocidadEnemigos = { facil: 0.75, normal: 1.00, dificil: 1.50, extremo: 2.00 };
  const dificultadVelocidad = obtenerAjuste('velocidadEnemigos') || 'normal';
  const multiplicadorVelocidad = multiplicadoresVelocidadEnemigos[dificultadVelocidad] ?? 1.00;
  let speedAdaptada = ((state.gameStructure === "arcade") ? 0.20 : 0.15) * multiplicadorVelocidad;
...
const VISTAS_MENU = ["view-structure", "view-vocabulary", "view-prepartida", "view-settings", "view-manual", "view-custom"];
...
function obtenerConfiguracionPartidaActual() {
  return {
    velocidadEnemigos: obtenerAjuste('velocidadEnemigos') || 'normal',
    maxEnemigos: state.MAX_ENEMIES,
...
function obtenerCombinacionPartidaActual() {
  return {
    nivel: state.currentMode,
    modo: state.gameStructure,
    ...obtenerConfiguracionPartidaActual(),
    dificultad: obtenerAjuste('velocidadEnemigos') || 'normal'
  };
}

function obtenerDescripcionCombinacionActual() {
  const c = obtenerCombinacionPartidaActual();
  const extras = c.modo === 'fases'
    ? `· Fase: ${c.palabrasPorFase} · Guardián: ${c.palabrasGuardian} · Jefe: ${c.palabrasJefeFinal}/${c.frasesJefeFinal}`
    : `· Guardián cada: ${c.arcadeKillsGuardian}`;
  return `Combinación: ${c.nivel} · ${c.modo} · ${c.dificultad} · Máx. enemigos: ${c.maxEnemigos} ${extras}`;
}

function registrarResultadoActual() {
  const dificultad = obtenerAjuste('velocidadEnemigos') || 'normal';
  const resultado = {
    nivel: state.currentMode, modo: state.gameStructure, dificultad,
    velocidad: dificultad, maxEnemigos: state.MAX_ENEMIES,
    duracionMs: state.tiempoPartidaMs || 0, puntos: state.score || 0,
    enemigosEliminados: state.kills || 0,
    guardianes: sistemaLector.miniJefesDerrotados || 0,
    palabrasJefe: state.gameStructure === 'fases' ? (state.estadisticasPartida?.palabrasJefe || 0) : 0,
    frasesJefe: state.gameStructure === 'fases' ? (state.estadisticasPartida?.frasesJefe || 0) : 0,
    palabrasGuardianArcade: state.gameStructure === 'arcade' ? (state.estadisticasPartida?.palabrasGuardianArcade || 0) : 0,
    fases: state.gameStructure === 'fases' ? (state.estadisticasPartida?.fases || []) : undefined,
    hitos: state.gameStructure === 'arcade' ? (state.estadisticasPartida?.hitos || []) : undefined
  };
  registrarResultadoPartida(resultado);
}
...
function update(dtFactor = 1) {
  ...
  // Cronómetro de partida: tiempo real, independiente de los FPS.
  const ahoraTiempoPartida = performance.now();
  if (state.ultimoTiempoPartidaMs == null) {
    state.ultimoTiempoPartidaMs = ahoraTiempoPartida;
  } else {
    state.tiempoPartidaMs = (state.tiempoPartidaMs || 0) +
      Math.max(0, ahoraTiempoPartida - state.ultimoTiempoPartidaMs);
    state.ultimoTiempoPartidaMs = ahoraTiempoPartida;
  }
  if (state.tiempoPartidaMs - (state.ultimoGuardadoPartidaMs || 0) >= 5000) {
    guardarPartidaActual(obtenerConfiguracionPartidaActual(), sistemaLector, controladorModoArcade);
    state.ultimoGuardadoPartidaMs = state.tiempoPartidaMs;
  }
if (state.isWinning) {
    actualizarParticulas(dtFactor);
    return;
...
if (state.gameStructure !== "arcade") {
     if (eraJefeFinal) {
        ...
        // Congelar el cronómetro en el instante exacto de la victoria.
        state.ultimoTiempoPartidaMs = null;
        target.y = -9999; 
        target.jp = ""; 
        target.romaji = "";
      ...
      if (tieneMasPalabras) {
        if (!state.estadisticasPartida) state.estadisticasPartida = { fases: [], hitos: [] };
        state.estadisticasPartida.fases.push(sistemaLector.palabrasSuperadasFase.length);
...
function togglePause() {
  ...
  if (state.paused) {
    ...
    state.ultimoTiempoPartidaMs = performance.now();
    btnPausa.innerHTML = "⏸️ Pausa"; msg.style.display = "none"; mobileInput.focus();
    mp3.play();
  }
}

function formatearTiempoMs(ms) {
...
function endGame(enemigoLetal) {
  state.gameOver = true;
  // Una partida terminada nunca puede quedar disponible para CONTINUAR.
  // Se elimina antes de cualquier otra transición para impedir que el autosave
  // de 5 segundos o beforeunload conserve una partida ya perdida.
  borrarPartidaGuardada(state.currentMode, state.gameStructure);
  ...
  const { record, esNuevo } = registrarPuntuacion(obtenerCombinacionPartidaActual(), state.score, state.tiempoPartidaMs);
  registrarResultadoActual();
  datosGameOver = { lineaRecord: `Récord: ${record?.puntos ?? 0} · Tiempo récord: ${formatearTiempoMs(record?.tiempoMs)}${esNuevo ? " 🏅 ¡Nuevo récord!" : ""}` };
...
function mostrarMensajeGameOver() {
  ...
  const tiempoTotalSegundos = Math.floor((state.tiempoPartidaMs || 0) / 1000);
  ...
  msg.innerHTML = `GAME OVER<br>${obtenerDescripcionCombinacionActual()}<br>Modo: ${modeName}<br>Puntos: ${state.score}<br>Tiempo: ${textoTiempo}<br>${lineaRecord}<br><button id="retry">Reintentar</button> <button id="changeMode">Cambiar modo</button>`;
...
function winGame() {
  ...
  registrarResultadoActual();
  borrarPartidaGuardada(state.currentMode, state.gameStructure);
  const lineaRecord = `Récord: ${record?.puntos ?? 0} · Tiempo récord: ${formatearTiempoMs(record?.tiempoMs)}${esNuevo ? " 🏅 ¡Nuevo récord!" : ""}`;
  const tiempoTotalSegundos = Math.floor((state.tiempoPartidaMs || 0) / 1000);
  const minutos = Math.floor(tiempoTotalSegundos / 60);
  const segundos = tiempoTotalSegundos % 60;
  const textoTiempo = `${String(minutos).padStart(2, "0")}:${String(segundos).padStart(2, "0")}`;

  msg.innerHTML = `🏆 ¡NIVEL FINALIZADO! 🏆<br>${obtenerDescripcionCombinacionActual()}<br>
                   ¡Felicidades, has dominado todo el vocabulario!<br><br>
                   Puntos totales: ${state.score}<br>Tiempo: ${textoTiempo}<br>${lineaRecord}<br><br>
...
function startGame(mode, partidaGuardada = null) {
  if (mode && MODES[mode]) {
    if (!partidaGuardada) borrarPartidaGuardada(mode, state.gameStructure);
    ...
    state.ultimoTiempoPartidaMs = performance.now();
    if (partidaGuardada) {
      // La configuración guardada queda fijada para esta continuación.
      const cfg = partidaGuardada.configuracion || {};
      for (const clave of ['velocidadEnemigos','maxEnemigos','palabrasPorFase','palabrasGuardian','palabrasJefeFinal','frasesJefeFinal','arcadeKillsGuardian']) {
        if (cfg[clave] !== undefined) fijarAjuste(clave, cfg[clave]);
      }
      state.MAX_ENEMIES = Number(cfg.maxEnemigos) || state.MAX_ENEMIES;
      restaurarSnapshotPartida(partidaGuardada, sistemaLector, controladorModoArcade);
    }
...
function showMenu() {
  if (state.started && !state.gameOver) guardarPartidaActual(obtenerConfiguracionPartidaActual(), sistemaLector, controladorModoArcade);
  state.started = false;
  state.paused = false;
  mp3.pause();
...
document.querySelectorAll("#view-vocabulary button[data-mode]").forEach(btn => {
  btn.addEventListener("click", () => {
    prepararPartida(btn.dataset.mode);
  });
});
...
// Protección adicional: intenta conservar la partida si se cierra/recarga la página.
window.addEventListener('beforeunload', () => {
  if (state.started && !state.gameOver && !state.isWinning) {
    guardarPartidaActual(obtenerConfiguracionPartidaActual(), sistemaLector, controladorModoArcade);
  }
});
